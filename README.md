...
Reviews and self-directed audits on codebases outside the standard EVM DeFi space — included to show range across chain architectures, even where no qualifying finding resulted.

| Protocol | Type | Scope | Outcome |
|---|---|---|---|
| Wormhole (Wormchain) | Immunefi bug bounty | `x/tokenfactory` module (Cosmos SDK) — minting authorization, storage-key derivation, cross-module and CVE-adjacent attack surface | No qualifying finding — [full methodology report](./self-audits/wormchain-tokenfactory/report.md) |

The Wormchain review is written up in full because the negative result is itself informative: it walks through 10 distinct attack hypotheses against the module and the specific code paths traced to rule each one out, rather than a summary claim of "reviewed, found nothing."
